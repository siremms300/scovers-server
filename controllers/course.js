...
export const uploadImage = async(req, res)=>{
  ...
  try {
      ...
      // image params 
      const params = {
          Bucket: "eagle-eye-bucket-images",
          Key: `${nanoid()}.${type}`,
          Body: base64Data,
          ACL: 'public-read',
...
export const subEnrollment = async(req, res)=> {
...
export const uploadEbook = async (req, res) => {
  try {
    ...
    if (!ebook) return res.status(400).send("No ebook found");

    // eBook params 
    const params = {
      Bucket: "eagle-eye-bucket-images",
      Key: `${nanoid()}.pdf`, // Generate a unique key for the PDF
      Body: readFileSync(ebook.path),
      ACL: "public-read",
...
export const saveEbook = async (req, res) => {
  try {
    const { slug, instructorId } = req.params;
    const { ebookInfo } = req.body;
    ...
    const updated = await Course.findOneAndUpdate(
      { slug },
      {
        $set: { ebook: ebookInfo }, // Assuming 'ebook' is a field in your Course model
      },
      { new: true }
    )
    ...
    console.error(err);
    return res.status(400).send('Ebook save failed');
  }
};
